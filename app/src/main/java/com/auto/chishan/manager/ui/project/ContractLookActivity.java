 ...
 * @author:DingZhixiang
 */
public class ContractLookActivity extends BaseResultActivity {
            ...
            public void onItemClick(View itemView, int viewType, int position) {
                switch (selectType) {
                    case 1:
                        ...
                        break;
                    case 2:
                        flowTv.setText(selectAdapter.getData().get(position).getName());
                        flow = selectAdapter.getData().get(position).getCode();
                        break;
                }
                popupLayout.dismiss();
            }
    ...
    private void showContractInfo() {
        ...
        OkGo.<LzyResponse<ContractLookDetailBean>>post(Urls.FirstHost + Urls.SecondHost).tag(this)
                .params("params", hexStr)
                .execute(new DialogCallback<LzyResponse<ContractLookDetailBean>>(mActivity, true) {
                    @Override
                    public void onSuccess(Response<LzyResponse<ContractLookDetailBean>> response) {
                        detailBean = response.body().getAuto_data();
                        contractNoTv.setText(detailBean.getContractNo());
                        moneyMaxEt.setText(detailBean.getContractAmount());
                        moneyMaxEt.setEnabled(false);
                        contractMoneyEt.setText(detailBean.getContractSignAmount());
                        contractMoneyEt.setEnabled(false);
                        contractTypeTv.setText(detailBean.getContractTempType());
                        contractTypeRl.setEnabled(false);
                        flowTv.setText(detailBean.getFlowType());
                        flowRl.setEnabled(false);
                        contractStartDateTv.setText(detailBean.getContractStartDate());
                        contractStartDateRl.setEnabled(false);
                        contractEndDateTv.setText(detailBean.getContractEndDate());
                        contractEndDateRl.setEnabled(false);
                        if(detailBean.getContractDetailList() != null){
                            contractBeans.addAll(detailBean.getContractDetailList());
                            adapter.notifyDataSetChanged();
                        }
                    }

                    @Override
                    public void onError(Response<LzyResponse<ContractLookDetailBean>> response) {
    ...
    private void save() {
        ...
        String hexStr = RSAandDES.getRSA("Android", RSAandDES.privateKey, spUtils.getLoginName(), spUtils
                .getPwd(), Urls.saveContractInfo, paramJo.toString());
    ...
    private void getContractType() {
        ...
        beanAccount1.setName("草拟合同");
        beanAccount1.setCode("true");
        selectBeans.add(beanAccount1);

        SelectBean beanAccount2 = new SelectBean();
        beanAccount2.setName("格式合同");
        beanAccount2.setCode("false");
    ...
    private void getFlow() {
        selectBeans.clear();
        SelectBean beanAccount1 = new SelectBean();
        beanAccount1.setName("非印刷合同");
        beanAccount1.setCode("2");
        selectBeans.add(beanAccount1);

        SelectBean beanAccount2 = new SelectBean();
        beanAccount2.setName("印刷合同");
