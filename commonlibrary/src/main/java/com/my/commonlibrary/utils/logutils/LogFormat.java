...
public class LogFormat {
    ...
    public static String formatBorder(String[] segments) {
        ...
        msgBuilder.append(TOP_HORIZONTAL_BORDER).append(MPrinter.lineSeparator);
        for (int i = 0; i < nonNullCount; i++) {
            msgBuilder.append(appendVerticalBorder(nonNullSegments[i]));
            if (i != nonNullCount - 1) {
                msgBuilder.append(MPrinter.lineSeparator).append(DIVIDER_HORIZONTAL_BORDER)
                        .append(MPrinter.lineSeparator);
            } else {
                msgBuilder.append(MPrinter.lineSeparator).append(BOTTOM_HORIZONTAL_BORDER);
            }
        }
    ...
    private static String appendVerticalBorder(String msg) {
        ...
        String[] lines = msg.split(MPrinter.lineSeparator);
        for (int i = 0, N = lines.length; i < N; i++) {
            if (i != 0) {
                borderedMsgBuilder.append(MPrinter.lineSeparator);
            }
            String line = lines[i];
